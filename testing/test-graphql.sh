...
time curl -s -X POST \
  ...
  -d '{"query":"query { bm020AuctionModels(limit: 100) { edges { node { auction_id name status } } } }"}' \
  "https://api.cartridge.gg/x/lax/torii" -o /tmp/auction-100.json
...
time curl -s -X POST \
  -H "Content-Type: application/json" \
  -d '{"query":"query { bm020AuctionModels(limit: 1000000) { edges { node { auction_id name status } } } }"}' \
  "https://api.cartridge.gg/x/lax/torii" -o /tmp/auction-1m.json
